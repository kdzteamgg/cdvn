...
local function CreateWindow(title)
    ...
    local openTween = CreateTween(mainFrame, {Size = UDim2.new(0, 350, 0, 200), Position = UDim2.new(0.5, -175, 0.5, -100)}, 0.5)
    openTween:Play()
...
local function InitializeUI()
    ...
    -- Create key label
    mainWindow:CreateLabel("NHẬP KEY ĐỂ ĐĂNG NHẬP:")
    ...
    -- Key verification function with automatic clear on failure
    local function verifyKey()
        if _G.EnteredKey and _G.EnteredKey ~= "" then
            ...
            if response == trueData then
                ...
                mainWindow:Notify("BYPASS", "LOADING", 4483362458, 5)
                LoadMainScript() -- This function should be defined elsewhere to load the main script
                SGGui:Destroy()
            else
                mainWindow:Notify("KHONG THANH CONG", "Key cua ban khong hop le. Vui long nhap lai.", 4483362458, 3.5)
                -- Clear textbox for retry
                keyTextbox.Set("")
                _G.EnteredKey = ""
            end
        else
    ...
    local verifyButton = mainWindow:CreateButton("XAC NHAN KEY", verifyKey)
    
    -- Add event to verify key when Enter is pressed in the textbox
    keyTextbox.TextBox.FocusLost:Connect(function(enterPressed)
