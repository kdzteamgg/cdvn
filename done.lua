...
local function CreateWindow(title)
    ...
    mainFrame.Size = UDim2.new(0, 350, 0, 180) -- Reduced height since we removed elements
    ...
    local openTween = CreateTween(mainFrame, {Size = UDim2.new(0, 350, 0, 180), Position = UDim2.new(0.5, -175, 0.5, -90)}, 0.5)
    openTween:Play()
    ...
    -- Create TextBox with auto-check functionality
    function window:CreateTextbox(text, placeholder)
        ...
        -- Auto-check on paste or text changed
        textbox:GetPropertyChangedSignal("Text"):Connect(function()
            local keyText = textbox.Text
            _G.EnteredKey = keyText
            
            -- Auto-check if key is longer than 30 characters
            if #keyText > 30 then
                local response = KeyGuardLibrary.validateDefaultKey(keyText)
                
                if response == trueData then
                    window:Notify("THANH CONG !!!", "LOAD SCRIPT...", 4483362458, 1.5)
                    ...
                    -- Delay before showing success notification and shrinking
                    ...
                    window:Notify("KHONG THANH CONG", "Key cua ban khong hop le. Hay paste key khac.", 4483362458, 3)
                    -- Clear textbox after validation failed
                    task.delay(0.5, function()
                        textbox.Text = ""
                    end)
                end
